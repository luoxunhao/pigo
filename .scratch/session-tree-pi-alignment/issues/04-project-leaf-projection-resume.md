...
- [ ] `leaf = explicitLeafID ?? lanes.main.leaf_id`；null 返回空会话
- [ ] 路径投影只保留最新 compaction 及其后 entries，并展开 retainedTail
- [ ] model/thinkingLevel 从路径上最新 model_change/thinking_level_change/assistant 推导
- [ ] 只有该入口能构建 `AgentContext.Messages`
- [ ] REPL/TUI/headless/serve/ACP resume 不再用文件末行推断 leaf
- [ ] `/tree N` 持久化 `moveLane("main", target)`，不再写 `customMetadata["curLeaf"]`
- [ ] `lanes.leaf_id` 悬空 fail-closed
- [ ] 跨前端恢复同一分支的集成测试通过
