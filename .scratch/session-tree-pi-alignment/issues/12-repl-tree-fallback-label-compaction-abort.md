...
- [x] `/tree` 输出编号树，`/tree N` 与 TUI 同 leaf 语义
- [x] 选中 user/custom_message 预填输入，可编辑后回车；当前 leaf 打印 `Already at this point`
- [x] `/label` 与 `/tree` 共用编号，空文本删除 label
- [x] branch summary 三选一与 custom prompt 输入；`--summary` 可覆盖配置
- [x] 压缩期间阻塞读输入，Ctrl+C 中止并回到提示符
- [x] typed-ahead 保留，中止后不主动清空
- [x] 文本 fallback 与 TUI 行为一致
