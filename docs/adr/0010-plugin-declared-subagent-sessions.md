...
isolation 默认 goroutine，process 显式声明且只用 builtin 工具；嵌套默认禁止，由 agent 声明 `nested/maxDepth` 并由 core depth 守卫。完整设计见 `tasks/spec/spec-012-session-tree-pi-alignment.md` §10。
