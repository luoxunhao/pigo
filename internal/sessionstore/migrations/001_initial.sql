...
CREATE TRIGGER IF NOT EXISTS trg_entries_fts_insert AFTER INSERT ON entries
BEGIN
	INSERT INTO session_search_fts(session_id, entry_id, seq, kind, text)
	SELECT new.session_id, new.id, new.seq, new.type, new.payload
	WHERE new.type IN ('message', 'custom_message', 'branch_summary', 'compaction');
END;

CREATE TRIGGER IF NOT EXISTS trg_entries_fts_delete AFTER DELETE ON entries
BEGIN
	DELETE FROM session_search_fts WHERE session_id = old.session_id AND entry_id = old.id;
END;

CREATE TRIGGER IF NOT EXISTS trg_entries_fts_update AFTER UPDATE ON entries
BEGIN
	DELETE FROM session_search_fts WHERE session_id = old.session_id AND entry_id = old.id;
	INSERT INTO session_search_fts(session_id, entry_id, seq, kind, text)
	SELECT new.session_id, new.id, new.seq, new.type, new.payload
...
CREATE TRIGGER IF NOT EXISTS trg_facts_fts_insert AFTER INSERT ON facts
BEGIN
	INSERT INTO session_search_fts(session_id, entry_id, seq, kind, text)
	SELECT new.session_id, 'fact-' || new.seq, new.seq, new.kind, coalesce(new.value, '')
	WHERE new.kind IN ('name', 'label');
END;

CREATE TRIGGER IF NOT EXISTS trg_facts_fts_delete AFTER DELETE ON facts
BEGIN
	DELETE FROM session_search_fts WHERE session_id = old.session_id AND entry_id = 'fact-' || old.seq;
END;

CREATE TRIGGER IF NOT EXISTS trg_facts_fts_update AFTER UPDATE ON facts
BEGIN
	DELETE FROM session_search_fts WHERE session_id = old.session_id AND entry_id = 'fact-' || old.seq;
	INSERT INTO session_search_fts(session_id, entry_id, seq, kind, text)
	SELECT new.session_id, 'fact-' || new.seq, new.seq, new.kind, coalesce(new.value, '')
