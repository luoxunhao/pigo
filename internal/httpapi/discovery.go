...
func discoverModelList(baseURL, apiKey, protocol string) ([]gen.DiscoveredModel, error) {
	...
	out := make([]gen.DiscoveredModel, 0, len(raw))
	...
	for _, m := range raw {
		...
		out = append(out, gen.DiscoveredModel{
			ModelId:        id,
			Name:           name,
